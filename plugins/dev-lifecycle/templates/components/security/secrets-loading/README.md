...
exposes:
  ...
  - SecretNotFoundError / MissingSecretsError — the two exception types callers catch
...
A framework-neutral, drop-in `secrets.py`: layered secret resolution (process
env first, an optional AWS Secrets Manager fallback second), a single typed
accessor, a startup fail-fast validator, and a hard rule against ever logging
a secret's value. Lives at
`templates/components/security/secrets-loading/` in this repo; Stage 3-4
backend blocks copy `secrets.py` verbatim into `app/core/security/secrets.py`.
Embodies `references/security/secrets-management.md` and the "Secrets never
in code or images" section of `references/security/secure-baseline.md` — this
...
- `SecretNotFoundError`, `MissingSecretsError` — the two exception types; both
  carry only the secret's **name**, never a value, in their message.
- Its co-located doc fragment: `docs/fragment.md`.
...
2. If `SECRETS_BACKEND=aws-secrets-manager`, AWS Secrets Manager, prefixed by
   ...
   top-level key by name; a scalar secret resolves as-is.
...
`tests/test_secrets.py` covers: env resolution, the empty-string-is-unset
...
priority (env beats ASM), the JSON-blob key-extraction case, and the lazy
`boto3` import path itself (a fake `boto3` module planted in `sys.modules`,
confirming `_get_asm_client()` reaches `boto3.client("secretsmanager", ...)`
correctly without a real `boto3` installed).
...
- **Module named `secrets.py` shadows the stdlib `secrets` module** once this
  component's directory is on `sys.path` (as the tests' `conftest.py` does,
  and as a scaffolded project's own import path will once it's copied to
  `app/core/security/secrets.py`). This is the name the build issue
  specified and the name a consuming project's own import (`from
  app.core.security.secrets import get_secret`) expects; the shadow is
  contained to modules that explicitly import from this component's
  directory or package path, not the whole interpreter. Flagged here rather
