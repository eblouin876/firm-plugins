...
- **23 skills** spanning the whole project lifecycle — `technical-proposal` → `product-planning` → `scaffolding` / `onboarding` → `planning` → `ui-exploration` / `design-system` → `backend` / `frontend` → `data` → `copywriting` → `testing` → `code-review` → `devops` / `infrastructure` → `dependency-maintenance` / `security-audit` → `documentation` / `debugging` / `walkthrough` — plus `coding-session`, the conductor that runs the whole plan → build → review → merge loop for you, and `template-author` / `recipe-author`, the two meta-skills for extending the plugin's own starter kit (below).
