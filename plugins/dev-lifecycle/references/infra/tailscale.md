...
## Core
- `tailscale up` joins a device to the tailnet; **MagicDNS** gives every node a stable name (`goatenheim`, etc.) — use names, not IPs.
...
- **Disable key expiry on always-on nodes** (Goatenheim, cloud hosts) in the admin console. Otherwise the key lapses on a schedule and the box silently drops off the tailnet — and if you're on the road, you're locked out. This is the single most common self-inflicted outage.
