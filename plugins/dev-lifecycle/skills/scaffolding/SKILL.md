...
### 5. Wire the pipeline
- **Claude GitHub App + workflows:** copy the firm's maintained workflow templates from `${CLAUDE_PLUGIN_ROOT}/assets/workflows/` — `claude.yml` (implement: responds to `@claude` on issues/PRs) and `claude-review.yml` (review: PR-opened → the review agent) — into the repo's `.github/workflows/`, replacing the `<owner>` placeholder with the marketplace repo owner. Install the Claude GitHub App (`/install-github-app`) so the workflows have a GitHub identity.
