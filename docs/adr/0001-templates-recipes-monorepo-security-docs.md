...
## Decision
Introduce two new artifact types, a monorepo convention, and two cross-cutting standards into the plugin — items (a)–(e) below — all under the existing self-extending-library model (metadata headers, `last-verified`, PR-gated changes):
...
- **(d) A standardized security baseline** — four new references (`references/security/secure-baseline.md`, `secrets-management.md`, `payments-security.md`, `data-protection.md`) that every block and recipe inherits by default, composing with the pre-existing `owasp.md` and `attack-surfaces.md` — anchored to OWASP Top 10:2025 / ASVS 5.0 — so "secure-by-default" is a bar authoring enforces, not a checklist a scaffolded project has to remember.
