...
// Vitest config for the `web` app — TEST-ONLY. Neither `next dev` nor
...
// so running `pnpm --filter web test` still spins up a Vite instance purely
...
// plugin the Vite SPA block wires into its OWN build for a related but
...
// Vite SPA's vite.config.ts test block — jsdom gives `document`/`window` so
// React renders headlessly and the api-client mutator's cookie-mode CSRF
// echo (which reads `document.cookie`) works, and the fixed `http://localhost`
// origin lets MSW match absolute handler URLs. `css: false` skips CSS
// processing in tests (no test imports the Tailwind entry / app/globals.css).
//
...
// zero extra config — exactly how the Vite SPA's vite.config.ts resolves the
// same two packages (no aliases there either). Run `pnpm --filter
// @repo/api-client build && pnpm --filter @repo/web-shared build` (or a full
// `pnpm install` + workspace build) before `pnpm --filter web test` if
