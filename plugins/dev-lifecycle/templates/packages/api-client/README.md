...
## Materialized-location paths
`tsconfig.json`'s `extends` and `eslint.config.mjs`'s import of the root config are both written as `../../<file>` — correct for this package's **materialized** location (`<project>/packages/api-client/`, two levels below the project root), not for where this file sits inside `eblouin-plugins` (`templates/packages/api-client/`, a sibling of `templates/monorepo/`). Don't "fix" these to be eblouin-plugins-relative; they're intentionally scaffolding-relative. See the inline comment at the top of each file.
